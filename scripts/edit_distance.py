...
def highest_match_action(ic, dc, sc, im, dm, sm, cost) -> str:
    ...
    * sm - substitution match (score)
    ...
    if max_match == sm and cost <= 0:
        best_action = EQUAL
        lowest_cost = sm
    elif max_match == sm and cost >= 0.5:
        best_action = REPLACE
        lowest_cost = sm
...
def edit_distance_backpointer(
    ...
    for i in range(1, m + 1):
        ...
        for j in range(1, n + 1):
            ...
            result = test(seq1[i - 1], seq2[j - 1])
            if type(result) is bool:
                cost = 0 if result else 1
                match_score = int(not cost)
            elif type(result) is int or type(result) is float:
                cost = result  # This result should be the lower the better
                if cost < 0:
                    match_score = 1 + abs(cost)
                else:  # cost == 0 or cost == 1
                    match_score = 1 - cost
            else:
                raise NotImplementedError()
