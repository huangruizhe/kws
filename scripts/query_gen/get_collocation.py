...
def lemmatize(lines, start_index=0, show_progress=True):
    ...
    for line in iter_obj:
        ...
        l_lemma = " ".join([wordnet_lemmatizer.lemmatize(w) for w in tokenization[start_index:]])
        if start_index > 0:
            l_lemma = " ".join(tokenization[:start_index]) + " " + l_lemma
...
def get_candidate_stems(opts, encoding):
    ...
    if order == 1:
        ...
        outputfile = Path(workdir) / f"lemma_candidates.{opts.data_name}.{order}.txt"
